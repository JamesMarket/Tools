...
def main():
    ...
    try:
        ...
        if success:
            # 获取原始值
            original_value = context.current_row.get('产品图片', [])
            print(f"原始值: {original_value}")
            
            # 更新图片列
            context.current_row['产品图片'] = [result]
            print(f"已设置新值: {[result]}")
            
            # 再次获取值确认更新
            updated_value = context.current_row.get('产品图片', [])
            print(f"更新后的值: {updated_value}")
            
            # 确认更新是否成功
            if updated_value == [result]:
                print("更新成功确认")
            else:
                print("警告：更新可能未生效")
